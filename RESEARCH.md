...
- Onboard `GridCoordinateTracker`: 첫 안정 node를 local `(0,0)`으로 두는 상대 좌표 기록, camera-relative branch mask의 grid-relative 변환, 현재는 드론 상태머신 전까지 vision-only heading 추정 포함
...
| `src/mission/GridCoordinateTracker.cpp` | 첫 node local `(0,0)` 기록, heading 기반 좌표 증가, camera branch mask의 grid-relative mask 변환. 현재는 드론 상태머신 전 수동/vision-only 테스트를 위한 기본 진입 heading과 snake용 pending second turn 추정 포함 |
...
| `src/vision/LineMaskBuilder.cpp` | ROI resize, grayscale, polarity별 mask 생성. `light_on_dark`는 `white_fill`, `dark_on_light`는 `dark_fill` 전용 경로로 넓은 밝은/어두운 라인을 각각 채워진 blob으로 만든 뒤 morphology clean 수행 |
| `src/vision/LineDetector.hpp` | line detector 선언 |
| `src/vision/LineDetector.cpp` | shared line mask 또는 image 입력 기반 contour scoring, anchor/lookahead band tracking point, polarity별 width limit, workload counter |
...
| `tests/test_intersection_detector.cpp` | synthetic image 기반 `+`, `T`, `L`, `straight`, dark/light polarity 분류 테스트와 `white_fill`/`dark_fill` 독립성 회귀 테스트. OpenCV target이 있을 때만 등록 |
...
| `tools/line_detector_tuner.cpp` | 이미지 파일 기반 line detector 튜닝/출력 tool. `--output`, `--white-*`, `--dark-*`, morphology/width override로 mask와 overlay 산출물 저장 |
...
- `line.mask_strategy = "white_fill"`은 현재 기본값이지만 polarity별로 분기된다. `light_on_dark`에서는 HSV low-saturation/high-value 기반 `white_fill`을 쓰고, `dark_on_light`에서는 value가 낮은 영역을 직접 잡는 `dark_fill` 경로로 떨어진다. 두 경로는 `white_*`와 `dark_*` 설정을 분리해 한쪽 튜닝이 다른 쪽 정확도를 흔들지 않게 한다.
- `white_v_min`, `white_s_max`, `fill_close_kernel`, `fill_dilate_kernel`, `max_line_width_ratio`는 밝은/흰색 라인 전용 튜닝값이다.
- `dark_v_max`, `dark_fill_close_kernel`, `dark_fill_dilate_kernel`, `dark_max_line_width_ratio`는 밝은 배경 위 검정/어두운 라인을 하나의 넓은 blob으로 잡기 위한 전용 튜닝값이다.
...
| `src/overlay/LineOverlay.cpp` | magenta contour, camera-center Y에 고정된 red line-center point, camera center부터 line center까지의 green horizontal offset primitive 생성. 표시 반응성을 위해 raw tracking point가 있으면 raw 값을 우선 사용 |
| `src/overlay/IntersectionOverlay.hpp` | intersection/decision overlay builder 선언 |
| `src/overlay/IntersectionOverlay.cpp` | 화면 상단/현재 접근 영역의 compact cyan `IX <type>` label과 yellow present branch ray primitive 생성. branch score와 긴 decision label은 영상에서 제거하고 log에 남김 |
...
| `src/telemetry/GridMapTracker.hpp` | `vision.grid_node` 기반 ASCII grid map tracker 선언 |
| `src/telemetry/GridMapTracker.cpp` | 방문 node, current heading marker, start line, 인접 edge를 누적해 `[grid-map]` 텍스트로 렌더링 |
...
| `src/ui/VisionLogWindow.cpp` | Windows vision log window 또는 stdout fallback. Win32에서는 상단 grid pane과 하단 telemetry pane을 분리하고, EDIT control용 CRLF 줄바꿈 정규화 수행 |
...
| `tests/test_grid_map_tracker.cpp` | `GridMapTracker` node 누적, edge 렌더링, 중복 id 무시 회귀 테스트 |
...
- 2026-05-03 GCS vision log window는 상단 `[grid-map]` pane과 하단 상세 telemetry pane으로 분리했고, Win32 multiline edit 줄바꿈 문제를 `\r\n` 정규화로 해결했다.
- 2026-05-03 `GridCoordinateTracker`는 드론 상태머신 전 수동 카메라 테스트를 위해 첫 node의 기본 heading과 snake row-end `pending_second_turn`을 추정한다. 이는 실제 MAVLink/IMU heading이 들어오기 전까지의 임시 debug 동작이다.
- 2026-05-03 `dark_on_light`에서 굵은 검정 라인이 edge만 잡히던 문제를 `dark_fill` 경로로 해결했다. 첨부 검정 라인 캡처 기준 offset은 약 `-62.3px`에서 `-14.1px`로 개선되었고, contour 후보 노이즈는 `427`개에서 `8`개로 감소했다. 밝은 라인 경로는 별도 `white_fill` 설정과 회귀 테스트로 독립성을 확인한다.
...
- `LineMaskBuilder`의 현재 기본 전략 이름은 `white_fill`이지만 실제 처리는 polarity별로 분리된다. `light_on_dark`는 HSV low-saturation/high-value 기반 `white_fill`로 넓은 밝은 라인 내부를 채우고, `dark_on_light`는 value가 낮은 영역을 직접 잡는 `dark_fill` 경로로 넓은 검정/어두운 라인을 하나의 blob으로 만든다.
...
- `LineDetector`는 line mode별 최대 폭 제한도 분리한다. 밝은 라인은 `max_line_width_ratio`, 어두운 라인은 `dark_max_line_width_ratio`를 사용해 한쪽 튜닝이 다른 쪽 후보 선택을 흔들지 않게 했다.
- `line_detector_tuner`는 `--output`을 받아 `mask_0.png`, `line_overlay.png`, `summary.txt`를 저장하고, `--white-v-min`, `--white-s-max`, `--fill-close`, `--fill-dilate`, `--dark-v-max`, `--dark-fill-close`, `--dark-fill-dilate`, `--dark-max-width`로 polarity별 튜닝값을 재현할 수 있다.
...
- 밝은 배경 위 검정/어두운 선 검출은 `dark_fill`로 분리되었지만, 실제 조명/노출이 달라지면 `dark_v_max`, `dark_fill_*`, `dark_max_line_width_ratio`를 다시 보정해야 한다.
- 배경과 대비되는 유색 라인은 명암 차이가 충분하면 현재 light/dark mode로 잡힐 수 있지만, 채도 높은 빨강/초록/파랑처럼 value만으로 구분이 애매한 라인은 별도 HSV hue 기반 `color_fill` 계열 전략이 필요할 수 있다.
...
- 수동 카메라 테스트에서 회전 완료를 확정하는 입력은 아직 없다. 실제 상태머신 전까지는 `l`/`r` 같은 debug turn-complete 입력을 onboard mission state에 넣는 방식을 검토할 수 있지만, 최종 비행에서는 IMU/MAVLink heading과 turn completion event가 기준이어야 한다.
...
6. 드론 없이 반복 검증할 기간에는 수동 `l`/`r` turn-complete debug 입력을 넣을지 결정한다. 구현한다면 GCS map을 직접 조작하지 않고 onboard 상태머신의 heading update 경로를 통해 `GridCoordinateTracker`에 반영한다.
7. 같은 조건에서 `--video --fps 12`를 켠 관제용 성능과 metadata-only 성능을 비교하되, 제어 loop 추가 전 기본값은 metadata-only/5FPS video cap으로 유지한다.
8. ArUco 50cm x 50cm marker를 실제 2m 거리에서 `aruco-only`, `aruco+line+intersection+decision` 각각 확인한다.
9. IMX519 `lens_position`, `autofocus_mode`, `exposure`, `shutter_us`, `gain`을 mission-like 조건에서 고정값 후보로 정한다.
10. 실제 고도/속도에서 `turn_zone_y_min/max`, `late_zone_y`, `record_node_once_frames`를 보정한다.
11. 그 다음 Pixhawk/MAVLink control loop를 onboard critical path로 구현한다.
