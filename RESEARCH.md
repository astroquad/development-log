...
- Protocol 문서는 v1.6이며 JSON top-level `protocol_version`은 호환성을 위해 여전히 integer `1`이다.
...
- GCS-side ArUco/line/intersection overlay
...
- Onboard system/camera/debug telemetry v1.6
- GCS vision log window와 packet/video/line/intersection/marker/system/camera 로그
...
| `docs/PROTOCOL.md` | onboard/GCS 공통 protocol v1.6 명세. `uav-gcs/docs/PROTOCOL.md`와 동일해야 함 |
...
| `src/app/VisionDebugPipeline.cpp` | rpicam frame read, JPEG decode, ArUco/line/intersection 실행, telemetry 생성/송신, optional latest-frame video worker, Pi system telemetry 수집 |
...
| `src/protocol/TelemetryMessage.hpp` | telemetry field 구조체: system, camera, vision, line, intersection, markers, grid, debug |
| `src/protocol/TelemetryMessage.cpp` | protocol v1 JSON serialization. 문서 버전 v1.6의 `vision.intersection` object도 포함 |
...
| `tests/test_telemetry_line_json.cpp` | line/intersection/debug/video telemetry JSON serialization 회귀 테스트 |
...
| `docs/PROTOCOL.md` | onboard/GCS 공통 protocol v1.6 명세. onboard protocol 문서와 동일해야 함 |
| `logs/.gitkeep` | runtime log 폴더 placeholder |
...
| `src/protocol/TelemetryMessage.hpp` | protocol v1.6 parser-side telemetry 구조체와 stats |
| `src/protocol/TelemetryMessage.cpp` | telemetry JSON parse, legacy field compatibility, structured intersection parse, stats update |
...
| `src/overlay/IntersectionOverlay.hpp` | intersection overlay builder 선언 |
| `src/overlay/IntersectionOverlay.cpp` | cyan center/type label, yellow present branch ray, branch score label primitive 생성 |
...
| `src/telemetry/VisionLogFormatter.cpp` | onboard processing timing, packet/video/system/camera/line/intersection/marker log 문자열 생성 |
...
| `tests/test_intersection_overlay.cpp` | intersection overlay primitive 회귀 테스트 |
| `tests/test_telemetry_line_parse.cpp` | telemetry v1.6 line/intersection/debug parse 회귀 테스트 |
...
| `debug.aruco_latency_ms`, `debug.line_latency_ms`, `debug.intersection_latency_ms` | detector별 처리 시간 |
...
- Onboard 기본 tests 통과: `telemetry_line_json`, `line_stabilizer`, `intersection_stabilizer`.
- Onboard OpenCV tests 통과: `telemetry_line_json`, `line_stabilizer`, `intersection_stabilizer`, `intersection_detector`.
...
- 과거 문서에는 protocol v1.5와 교차점 placeholder 설명이 남아 있었다. 현재는 protocol v1.6의 `vision.intersection` 구조체와 GCS overlay/log가 기준이다.
...
- 교차점 분류는 구현되었지만 밝은 배경, 넓은 선, threshold 주변 branch score, 회전된 격자에서 `unknown`/type downgrade가 남아 있다.
- Grid state는 아직 placeholder라 line/intersection metadata는 경로 판단 전 단계다.
...
3. 같은 조건에서 `--video --fps 12`를 켠 관제용 성능과 metadata-only 성능을 비교하되, 제어 loop 추가 전 기본값은 metadata-only/5FPS video cap으로 유지한다.
4. ArUco 50cm x 50cm marker를 실제 2m 거리에서 `aruco-only`, `aruco+line+intersection` 각각 확인한다.
5. IMX519 `lens_position`, `autofocus_mode`, `exposure`, `shutter_us`, `gain`을 mission-like 조건에서 고정값 후보로 정한다.
6. 교차점 결과를 grid coordinate와 snake search mission state로 연결한다.
